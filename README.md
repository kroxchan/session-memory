...
| **隐私安全** | `sessions/` 默认 `.gitignore`，不存密钥和 PII |
...
重启 Cursor，或直接说 `/session-memory` 唤醒。详见 [安装指南](docs/getting-started.md)。

---

## 与 SkillForge 的关系

Session Memory 和 SkillForge（[@kroxchan/skillforge](https://github.com/kroxchan/skillforge)）**完全正交**，共用同一 `memory/` 根目录：

| 维度 | SkillForge | Session Memory |
|------|-----------|----------------|
| 存什么 | Agent 能力评分、反思、delta | 项目需求、决策、会话摘要 |
| 维度 | `task_type`（跨项目） | `project`（当前 workspace）|
| 谁写 | `sf update-l0` 命令 | Agent 自动识别写入 |
| 何时读 | Phase 1 校准时 | 会话启动 + 对话中按需 |
| 用途 | "上次我做 X 类任务得了几分" | "这个项目用 Redis 不用 Memcached" |

两者共存，互不干扰。Session Memory 只往 `memory/sessions/` 写，不碰 SkillForge 的其他文件。
