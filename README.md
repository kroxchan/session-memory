...
> Agent 的项目级持久化记忆系统。防止长对话中信息丢失，支持跨窗口和 `/clear` 后恢复上下文。

Session Memory 让 Agent 在每个工作目录下拥有持久化的"记忆"——技术决策、需求约束、项目约定、历史讨论摘要——全都不依赖对话窗口，下次打开依然在。
...
```
用户打开项目 A（如 ~/code/my-project）
        │
        ▼
  sm-project-key.sh
  → 计算 project_key = my-project@a3f2e1b9
        │
        ▼
  读取 projects/my-project@a3f2e1b9/
...
| **通用兼容** | 适用于 Cursor、Codex、Claude Code 及任何基于 Bash 的 Agent |
...
重启 Agent 即可生效。详见 [docs/getting-started.md](docs/getting-started.md)。

### 适用工具

| 工具 | 安装方式 |
|------|---------|
| **Cursor** | `bash install.sh`，放入 `~/.cursor/skills/` |
| **Codex** | `bash install.sh`，放入 `$CODEX_HOME/skills/` |
| **Claude Code** | 手动将 `skill/` 目录放入 Agent 识读的 skills 目录 |
| **其他 Bash Agent** | 直接调用 `scripts/sm-*.sh` 脚本即可 |
